...
package org.cocos2dx.javascript.iap.util;
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
			...
			try {
				...
				obj.put("evt", "iap");
				obj.put("signedData", purchaseData);
				obj.put("signature", dataSignature);
				//CubeiaClient.getInstance().sendServicePacket(obj.toString());
//				AppActivity.save_ins.sendServicePacket(obj.toString());
				//SActivity.getCurrentScreen().showWaitingDialog();

				// TODO:
				if (!Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
					// if (!Security.verifyPurchase(mSignatureBase64, jpurchaseData.toString(), dataSignature)) {
					Log.w("PURCHASE", "purchase false");
					logError("Purchase signature verification FAILED for sku " + sku);
					result = new IabResult(IABHELPER_VERIFICATION_FAILED, "Signature verification failed for sku "
							+ sku);
					if (mPurchaseListener != null)
						mPurchaseListener.onIabPurchaseFinished(result, purchase);
					return true;
				} else {
					Log.w("PURCHASE", "purchase OK");
				}
            ...
            String str = "{\"functionName\":10305,\"params\":[\"UserCanceled:\"]}";
//            AppActivity.callOut(str);
            ...
            String str = "{\"functionName\":10305,\"params\":[\"PurchaseFailed:\"]}";
//            AppActivity.callOut(str);
	...
	void consume(Purchase itemInfo) throws IabException {
		...
		try {
			...
			if (token == null || token.equals("")) {
				...
				throw new IabException(IABHELPER_MISSING_TOKEN, "PurchaseInfo is missing token for sku: " + sku + " "
						+ itemInfo);
			}
	...
	public static String getResponseDesc(int code) {
		...
		String[] iabhelper_msgs = ("0:OK/-1001:Remote exception during initialization/"
				...
				+ "-1004:Send intent failed/" + "-1005:User cancelled/" + "-1006:Unknown purchase response/"
				+ "-1007:Missing token/" + "-1008:Unknown error/" + "-1009:Subscriptions not available/"
	...
	int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
		...
		do {
			...
			logDebug("\n");
			logDebug("ownedSkus size: " + ownedSkus.size());
			logDebug("purchaseDataList size: " + purchaseDataList.size());
			logDebug("signatureList size: " + signatureList.size());
