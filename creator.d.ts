...
declare module cc {	
	...
	*/
	...
	cc.NGWlog(cc.isValid(node));    // true
	node.destroy();
	cc.NGWlog(cc.isValid(node));    // true, still valid in this frame
	// after a frame...
	cc.NGWlog(cc.isValid(node));    // false, destroyed in the end of last frame
	``` 
	*/
	...
	*/
	...
	@example 
	```js
	cc.NGWlog(cc.colorEqual(cc.Color.RED, new cc.Color(255, 0, 0))); // true
	``` 
	*/
	...
	*/
	...
	export class TiledLayer extends _SGComponent {		
		...
		*/
		...
		cc.NGWlog("MapTile size: " + mapTileSize);
		``` 
		*/
		getMapTileSize(): Size;		
		/**
		...
		*/
		...
		cc.NGWlog("Layer Orientation: " + orientation);
		``` 
		*/
		...
		*/
		...
		cc.NGWlog("Properties: " + properties);
		``` 
		*/
		getProperties(): any[];		
		/**
	...
	export class TiledMap extends Component {		
		...
		*/
		...
		cc.NGWlog("Map Orientation: " + mapOrientation);
		``` 
		*/
		...
		*/
		...
		for (var i = 0; i < properties.length; ++i) {
		    cc.NGWlog("Properties: " + properties[i]);
		}
		``` 
		*/
		getProperties(): any[];		
	...
	export class Game extends EventTarget {		
		...
		1. debugMode<br/>
		     ...
		     1 - cc.error, cc.assert, cc.warn, cc.NGWlog will print in console.                      <br/>
		     2 - cc.error, cc.assert, cc.warn will print in console.                              <br/>
		     3 - cc.error, cc.assert will print in console.                                       <br/>
		     4 - cc.error, cc.assert, cc.warn, cc.NGWlog will print on canvas, available only on web.<br/>
		...
		1. debugMode（debug 模式，但是在浏览器中这个选项会被忽略）                                <br/>
		     "debugMode" 各种设置选项的意义。                                                   <br/>
		         0 - 没有消息被打印出来。                                                       <br/>
		         1 - cc.error，cc.assert，cc.warn，cc.NGWlog 将打印在 console 中。                  <br/>
		         2 - cc.error，cc.assert，cc.warn 将打印在 console 中。                          <br/>
		         3 - cc.error，cc.assert 将打印在 console 中。                                   <br/>
		         4 - cc.error，cc.assert，cc.warn，cc.NGWlog 将打印在 canvas 中（仅适用于 web 端）。 <br/>
	...
	export class Node extends _BaseNode {		
		...
		*/
		...
		cc.NGWlog("Running Action Count: " + count);
		``` 
		*/
		...
		@example 
		```js
		cc.NGWlog("Node Position: " + node.getPosition());
		``` 
		*/
		getPosition(): Vec2;		
		/**
		...
		*/
		...
		@example 
		```js
		cc.NGWlog("Node Scale: " + node.getScale());
		``` 
		*/
		getScale(): number;		
		/**
		...
		*/
		...
		@example 
		```js
		cc.NGWlog("Content Size: " + node.getContentSize());
		``` 
		*/
		...
		*/
		...
		@example 
		```js
		cc.NGWlog("Node AnchorPoint: " + node.getAnchorPoint());
		``` 
		*/
		getAnchorPoint(): Vec2;		
		/**
		...
		*/
		...
		@example 
		```js
		cc.NGWlog("AnchorPointInPoints: " + node.getAnchorPointInPoints());
		``` 
		*/
		...
		*/
		...
		@example 
		```js
		cc.NGWlog(node.isCascadeOpacityEnabled());
		``` 
		*/
	...
	export class Layout extends Component {		
		...
		cc.NGWlog(childNode.x); // not yet changed
		layout.updateLayout();
		cc.NGWlog(childNode.x); // changed
		``` 
		*/
		updateLayout(): void;		
	...
	export class Sprite extends _RendererUnderSG {		
		...
		*/
		...
		cc.NGWlog("Inset Left:" + insetLeft);
		``` 
		*/
		getInsetLeft(): number;		
		/**
		...
		*/
		...
		cc.NGWlog("Inset Right:" + insetRight);
		``` 
		*/
		...
		*/
		...
		cc.NGWlog("Inset Bottom:" + insetBottom);
		``` 
		*/
	...
	export class Widget extends Component {		
		...
		cc.NGWlog(widget.node.y); // not yet changed
		widget.updateAlignment();
		cc.NGWlog(widget.node.y); // changed
		``` 
		*/
		...
		*/
		...
		cc.EventListener.create({
		    ...
		    onKeyPressed: function (keyCode, event) {
		        cc.NGWlog('pressed key: ' + keyCode);
		    },
		    onKeyReleased: function (keyCode, event) {
		        cc.NGWlog('released key: ' + keyCode);
		    }
		});
	...
	export class loader extends Pipeline {		
		...
		*/
		...
		cc.loader.load('a.png', function (err, tex) {
		    cc.NGWlog('Result should be a texture: ' + (tex instanceof cc.Texture2D));
		});
		
		cc.loader.load('http://example.com/a.png', function (err, tex) {
		    cc.NGWlog('Should load a texture from external url: ' + (tex instanceof cc.Texture2D));
		});
		
		cc.loader.load({url: 'http://example.com/getImageREST?file=a.png', type: 'png'}, function (err, tex) {
		    cc.NGWlog('Should load a texture from RESTful API by specify the type: ' + (tex instanceof cc.Texture2D));
		});
		
		cc.loader.load(['a.png', 'b.json'], function (errors, results) {
		    if (errors) {
		        for (var i = 0; i < errors.length; i++) {
		            cc.NGWlog('Error url [' + errors[i] + ']: ' + results.getError(errors[i]));
		        }
		    }
		...
		*/
		...
		cc.loader.loadRes('misc/character/cocos', function (err, prefab) {
		    ...
		    cc.NGWlog('Result should be a prefab: ' + (prefab instanceof cc.Prefab));
		});
		...
		cc.loader.loadRes('imgs/cocos', cc.SpriteFrame, function (err, spriteFrame) {
		    ...
		    cc.NGWlog('Result should be a sprite frame: ' + (spriteFrame instanceof cc.SpriteFrame));
		});
		``` 
		*/
	...
	export class LoadingItems extends CallbacksInvoker {		
		...
		*/
		...
		loadingItems.onComplete = function (errors, items) {
		     if (error)
		         cc.NGWlog('Completed with ' + errors.length + ' errors');
		     else
		         cc.NGWlog('Completed ' + items.totalCount + ' items');
		 }
		``` 
		*/
		...
		cc.LoadingItems.create(cc.loader, ['a.png', 'b.plist'], function (completedCount, totalCount, item) {
		     ...
		     if (errors) {
		         for (var i = 0; i < errors.length; ++i) {
		             cc.NGWlog('Error url: ' + errors[i] + ', error: ' + items.getError(errors[i]));
		         }
		     }
		     else {
	...
	export class url {		
		...
		cc.NGWlog(url);   // "resources/raw/textures/myTexture.png"
	...
	export class Color extends ValueType {		
		...
		*/
		...
		cc.NGWlog(color1.equals(color2)); // true;
		color2 = cc.Color.RED;
		cc.NGWlog(color2.equals(color1)); // false;
		``` 
		*/
...
declare module sp {	
	...
	export class Skeleton extends cc._RendererUnderSG {		
		...
		cc.NGWlog(bone.worldX); // return 0;
		spine.updateWorldTransform();
		bone = spine.findBone('head');
		cc.NGWlog(bone.worldX); // return -23.12;
		``` 
		*/
