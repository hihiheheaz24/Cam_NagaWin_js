...
FIREBASE.Connector = function (packetCallback, lobbyCallback, loginCallback, statusCallback) {
    ...
    var _handleLoginResponse = function (loginResponse) {
        if (_loginCallback) {
            ...
            cc.NGWlog('-------------->  _handleLoginResponse   ', loginResponse);
            let status;
            let pid;
            let screenname;
            let credentials;
            let code;
            let message;
            if (loginResponse.hasOwnProperty('status')) {
                status = loginResponse.status;
            }
            cc.NGWlog('----> status ', status);
            if (loginResponse.hasOwnProperty('pid')) {
                pid = loginResponse.pid;
            }
            cc.NGWlog('----> pid ', pid);
            if (loginResponse.hasOwnProperty('screenname')) {
                screenname = loginResponse.screenname;
            }
            cc.NGWlog('----> screenname ', screenname);
            if (loginResponse.hasOwnProperty('credentials')) {
                credentials = loginResponse.credentials;
            }
           // cc.NGWlog('----> credentials ', credentials);
            if (loginResponse.hasOwnProperty('code')) {
                code = loginResponse.code;
            }
            cc.NGWlog('----> code ', code);
            if (loginResponse.hasOwnProperty('message')) {
                message = loginResponse.message;
            }
            cc.NGWlog('----> message ', message);

            _loginCallback(loginResponse.status, loginResponse.pid, loginResponse.screenname, loginResponse.credentials, loginResponse.code, loginResponse.message);
        }
    };
    ...
    var _handlePacket = function (protocolObject) {
        ...
        switch (protocolObject.classId) {
            ...
            // special case for cometd
            case FB_PROTOCOL.ForcedLogoutPacket.CLASSID:
                // pass the packet on to the client
                if (_packetCallback) {
                    _packetCallback(protocolObject);
                }

                if (_cometd === true) {
                    // immediateley close the cometd session too avoid
                    // automatic reconnects and race conditions
                    _ioAdapter.close();
            ...
            case FB_PROTOCOL.PingPacket.CLASSID:
                cc.NGWlog("received ping from firebase");
    ...
    //     var ping = new FB_PROTOCOL.PingPacket();
    //    // watchRequest.tableid = tableId;
    //     this.sendProtocolObject(ping);
    // }
    /**
    ...
    this.close = function () {
        try {
            ...
            cc.NGWlog("exception thrown when closing connection");
        }
    };
