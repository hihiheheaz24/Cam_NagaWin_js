var AlertView = cc.Class({
    ...
    setPosAlert() {
        ...
        } else if(require('GameManager').getInstance().getCurrentSceneName() !== CURRENT_VIEW.DT_VIEW) {
            let pos = cc.v2(cc.winSize.width / 10, cc.winSize.height / 2.23);
            this.bg.node.position = pos;
        }
        if (require("GameManager").getInstance().gameView !== null || require('GameManager').getInstance().getCurrentSceneName() === CURRENT_VIEW.DT_VIEW) {
            ...
            || require('GameManager').getInstance().getCurrentSceneName() === CURRENT_VIEW.DT_VIEW) {
